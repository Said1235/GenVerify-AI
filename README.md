...
**A note on `explanation` and `detected_fields` wording.** Because only
`status` is compared between the leader's and validator's independent
runs, free-text fields like `explanation` can never cause a consensus
failure in this contract, regardless of how differently two models
phrase the same finding — this was a deliberate design choice, not an
oversight. `detected_fields` is a different case: it isn't compared
either, but its *content* still matters for anyone consuming the
output, so the prompt explicitly asks for canonical `snake_case` field
labels (`"agreement_id"`, not `"Agreement ID: SA-2026-0098"` and not a
full sentence) to keep the output stable and machine-parseable.
